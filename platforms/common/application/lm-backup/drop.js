...
var singles = {
    disable: function(){
        ...
        if (mode == 'page'){
            ...
            if (sectionChildren) sectionChildren.style({'pointer-events': 'none'});
            return;
        }
        ...
        children.attribute('style', null).forEach(function(element){
            element = $(element);
            if (!element.siblings()) element.style({'pointer-events': 'none'});
        });
    },
    enable:  function(){
        var root = $('[data-lm-root]'),
        ...
        children.forEach(function(element){
            element = $(element);
            if (!element.siblings()) element.attribute('style', null);
        });
    },
    cleanup: function(builder){
        ...
        if (grids) grids.forEach(function(grid){
            grid     = $(grid);
...
var DropManager = new prime({
    ...
    start: function(event, element){
        ...
        this.original     = $(clone).after(element).style({display: 'block', opacity: 0.5}).addClass('original-placeholder').data('lm-dropzone', null);
        this.originalType = type;
        this.block        = get(this.builder.map, element.data('lm-id') || '') || new Blocks[type]();

        if (!this.block.isNew()){
            element.style({position: 'absolute', zIndex: 1000, width: element[0].offsetWidth, height: element[0].offsetHeight});
            ...
            case 'section':
                ...
                if (['left', 'right'].indexOf(position) == -1) this.placeholder[position](target);
                    ...
                    if (target.parent('.block').data('lm-id') || target.parent().data('lm-root')){
                        var grid  = zen('div.grid[data-lm-id="' + this.block.guid() + '"][data-lm-dropzone][data-lm-blocktype="grid"]').before(target),
    ...
    stop: function(event, target, element){
        ...
        if (type == 'block' && this.placeholder.siblings('.position, .spacer, .grid') && (children && children.length == 1)){
            ...
            resizeCase = {case: 2, siblings: block.block.siblings()};
            block.block.remove();
